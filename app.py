...
def dashboard():
    ...
    cur.execute("SELECT COUNT(*) as c FROM pagos WHERE date(fecha_pago) >= date('now','-30 day')")
    pagos_30 = cur.fetchone()["c"]
    cur.execute("SELECT COUNT(*) as c FROM clientes WHERE lower(estado_servicio) = 'corte'")
    en_corte = cur.fetchone()["c"]
...
def clientes_add():
    ...
    db = get_db()
    cur = db.cursor()
    cur.execute("INSERT INTO clientes (nombre,direccion,telefono,correo,estado_servicio) VALUES (?,?,?,?,?)",
                (nombre,direccion,telefono,correo,estado))
    db.commit()
    flash("Cliente agregado.", "success")
    return redirect(url_for("clientes"))

@app.route("/clientes/suspender/<int:cliente_id>")
...
def pagos_add():
    ...
    db = get_db()
    cur = db.cursor()
    cur.execute("INSERT INTO pagos (cliente_id,monto,fecha_pago,metodo_pago,mes_correspondiente,observaciones) VALUES (?,?,?,?,?,?)",
                (cliente_id, monto_f, fecha_str, metodo, mes, obs))
    db.commit()
...
def export_excel():
    ...
    ws = wb.active
    ws.title = "Pagos"
    ws.append(["ID","Cliente","Monto","Fecha","Método","Mes","Observaciones","Estado servicio"])
    for r in rows:
        ws.append([r["id"], r["cliente"], r["monto"], r["fecha_pago"], r["metodo_pago"], r["mes_correspondiente"], r["observaciones"], r["estado_servicio"]])
    bio = io.BytesIO()
    wb.save(bio)
    bio.seek(0)
